...
const TYPE_LABEL   = { casual: "Casual", training: "Training", tournament: "Tournament" };

export function EventModal({ event, onClose, onUpdate }) {
  ...
  async function handleJoin(e) {
    ...
    setSubmitting(false);
    if (insertError) {
      setError(insertError.code === "23505" ? "You've already joined this event." : insertError.message);
  ...
  async function handleCancelParticipation() {
    ...
    setSuccess(false);
    setParticipations((prev) => prev.filter((p) => p.user_id !== user.id));
  }
      ...
      <div className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-border bg-card shadow-2xl">
        <div className="p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {/* Clickable seats / participants badge */}
            <button
              type="button"
          ...
          {showParticipants && (
            <div className="mt-3 rounded-lg border border-border bg-muted/50 p-3">
              ...
              {isOrganizer && participations.length > 0 && (
                <p className="mt-2 text-xs text-muted-foreground">Visible to you as the organizer.</p>
              )}
            </div>
          )}
              ...
              {confirmCancel ? (
                <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-4">
                  <p className="text-sm font-medium text-destructive mb-3">
                    Cancel this event? All participants will see it as cancelled.
                  </p>
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 inline-flex h-9 items-center justify-center rounded-lg bg-destructive text-sm font-medium text-destructive-foreground hover:opacity-90 disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 inline-flex h-9 items-center justify-center rounded-lg border border-border bg-card text-sm font-medium text-card-foreground hover:bg-muted"
                    >
                      Keep it
                    </button>
                  </div>
          ...
          {isCancelled ? (
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="text-sm font-medium text-muted-foreground underline-offset-4 hover:underline hover:text-destructive disabled:opacity-50"
              >
            ...
            <form onSubmit={handleJoin} className="flex flex-col gap-4">
              ...
              <div>
                <label htmlFor="p-name" className="text-sm font-medium text-foreground">Full name</label>
                <input
                  id="p-name" type="text" required className={inputClass}
                  value={formData.full_name}
                  onChange={(e) => setFormData((p) => ({ ...p, full_name: e.target.value }))}
                />
              </div>
              <div>
                <label htmlFor="p-phone" className="text-sm font-medium text-foreground">Phone</label>
                <input
                  id="p-phone" type="tel" required className={inputClass}
